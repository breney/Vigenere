...
public class Vigenere extends HttpServ{
    ...
    public static String encoder(String word, String key) {
        ...
        return "Encoder = " + code + " || Key = " + key;
    }

    public static String decoder(String word, String key) {
        ...
        for (int k = 0; k < word.length(); k++) {
            ...
            for (int m = 0; m < alphabet.length; m++) {
                if (encryption[x][m] == Character.toUpperCase(wordChar[k])) {
                    y = m;
                    m = alphabet.length;
                }
            }
            index++;
            code += Character.toLowerCase(alphabet[y]);
        }

        return "Decoder = " + code + " || Key = " + key;
    }
